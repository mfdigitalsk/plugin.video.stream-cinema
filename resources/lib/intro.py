...
def intro(step=None, credentials_only=False):
    ...
    # ✅ Skip login if already configured
    if step is None and get_setting('kraska.user') != '':
        info('Skipping intro: Credentials already set.')
        return
    
    # ✅ Force skip steps 2-4 and directly go to step 5
    step = 5 if credentials_only is False else 0
...
def _remove_settings_file():
    try:
        fn = make_legal_filename(translate_path(f"special://profile/addon_data/{ADDON_ID}/settings.xml"))
        debug(f'REMOVE SETTINGS FILE: {fn} -> {xbmcvfs.exists(fn)}')
...
def introx(step=None, credentials_only=False):
